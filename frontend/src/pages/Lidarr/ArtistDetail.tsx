...
export function ArtistDetail() {
  ...
  const fanart = (artist as any).images?.find(
    (i: any) => i.coverType === 'fanart' || i.coverType === 'banner',
  )
      ...
      <div className="px-4 pb-4 flex gap-3 -mt-12 relative z-10">
        {poster && (
          <div className="w-20 h-20 rounded-full overflow-hidden ring-2 ring-slate-700 shrink-0">
            <img
              src={`${cfg?.baseUrl}${poster.url}`}
              alt={(artist as any).artistName}
        ...
        <div className="flex-1 pt-12">
          ...
          <p className="text-xs text-slate-500 mt-1">
            {(artist as any).statistics?.albumCount} albums ·{' '}
            {(artist as any).statistics?.trackFileCount} tracks
          </p>
        </div>
      </div>
          ...
          <div key={album.id} className="flex items-center gap-3 bg-slate-800/50 rounded-xl p-3">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-slate-500">
                {album.releaseDate?.slice(0, 4)} · {album.statistics?.trackCount} tracks
              </p>
            </div>
            <Badge
              variant={album.statistics?.percentOfTracks >= 100 ? 'success' : 'warning'}
              className="text-[10px]"
            >
