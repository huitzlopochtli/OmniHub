        ...
        {TABS.map((tab) => (
          <button key={tab.path} onClick={() => navigate(tab.path)}
            className={cn('px-4 py-2 text-sm whitespace-nowrap border-b-2 transition-colors',
              location.pathname === tab.path ? 'border-indigo-500 text-indigo-400' : 'border-transparent text-slate-400 hover:text-slate-200')}>
            {tab.label}
          </button>
        ))}
      </div>
...
function QueueView() {
  ...
  const { data: status } = useQuery({ queryKey: ['nzbget', 'status'], queryFn: () => nzbgetApi.status(), refetchInterval: 3000 })
  const pauseMut = useMutation({ mutationFn: nzbgetApi.pauseDownload, onSuccess: () => qc.invalidateQueries({ queryKey: ['nzbget'] }) })
  const resumeMut = useMutation({ mutationFn: nzbgetApi.resumeDownload, onSuccess: () => qc.invalidateQueries({ queryKey: ['nzbget'] }) })
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      <div className="shrink-0 flex items-center justify-between px-4 py-2 border-b border-slate-700/50 bg-slate-800/30">
        ...
        <Button size="sm" variant="ghost" onClick={() => paused ? resumeMut.mutate() : pauseMut.mutate()}>
          {paused ? <Play size={14} /> : <Pause size={14} />}
        </Button>
      </div>
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
        {(items as any[]).map((item: any) => {
          const pct = item.FileSizeMB > 0 ? ((item.FileSizeMB - item.RemainSizeMB) / item.FileSizeMB) * 100 : 0
          return (
        ...
        {!(items as any[]).length && <div className="text-center py-16 text-slate-500">Queue is empty</div>}
      </div>
    </div>
  )
}
...
function HistoryView() {
  const { data: items = [], isLoading, error, refetch } = useQuery({ queryKey: ['nzbget', 'history'], queryFn: () => nzbgetApi.history() })
  if (isLoading) return <div className="flex items-center justify-center h-full"><Spinner size="lg" /></div>
  if (error) return <ErrorState error={error} retry={refetch} />
  return (
      ...
      {(items as any[]).map((item: any) => (
        <div key={item.NZBID} className="px-4 py-3">
          ...
          <div className="flex items-center gap-2 mt-0.5">
            <Badge variant={item.Status === 'SUCCESS' ? 'success' : item.Status === 'FAILURE' ? 'danger' : 'default'} className="text-[10px]">{item.Status}</Badge>
...
export function NZBGetApp() {
  const enabled = useServiceEnabled('nzbget')
  if (!enabled) return <NotConfigured serviceName="NZBGet" />
  return (
