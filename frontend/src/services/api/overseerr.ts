...
export function createOverseerrApi(instanceId: string) {
  return {
    ...
    // TV detail
    getTVDetail: (tvdbId: number) => serviceGet<SeerrTVResult>(instanceId, `/api/v1/tv/${tvdbId}`),
    ...
    approveRequest: (requestId: number) =>
      servicePost(instanceId, `/api/v1/request/${requestId}/approve`, undefined),

    declineRequest: (requestId: number) =>
      servicePost(instanceId, `/api/v1/request/${requestId}/decline`, undefined),
