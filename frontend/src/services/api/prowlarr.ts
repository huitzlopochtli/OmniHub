...
export function createProwlarrApi(instanceId: string) {
  return {
    ...
    deleteIndexer: (id: number) => serviceDelete(instanceId, `/api/v1/indexer/${id}`, {}),
    ...
    search: (query: string, categories?: number[], indexerIds?: number[]) =>
      serviceGet<ProwlarrSearchResult[]>(instanceId, '/api/v1/search', {
        params: { query, ...(categories && { categories }), ...(indexerIds && { indexerIds }) },
      }),
...
export const prowlarrApi: ReturnType<typeof createProwlarrApi> = new Proxy(
  {} as unknown as ReturnType<typeof createProwlarrApi>,
  {
