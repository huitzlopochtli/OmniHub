...
8. **iOS PWA viewport height / scroll broken** — `100dvh` is unreliable on iOS PWA at startup. The fix is the `--app-height` pattern: `main.tsx` calls `updateAppHeight()` (sets `--app-height = window.innerHeight` on `<html>`) immediately and on `resize`/`orientationchange`. `body`, `#root`, and the AppShell outer div all use `height: var(--app-height, 100dvh)`. The BottomNav scroll clearance is provided by `--nav-offset` (in `:root` CSS) applied as `pb-[var(--nav-offset)] lg:pb-0` on the inner content wrapper in `AppShell.tsx`. **Do not revert to `h-dvh` or a `@media` CSS class** — both are unreliable on iOS PWA.
