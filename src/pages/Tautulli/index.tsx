...
const TABS = [{ path: '/activity', label: 'Activity' }, { path: '/history', label: 'History' }, { path: '/stats', label: 'Stats' }]
        ...
        {TABS.map((tab) => (
          <button key={tab.path} onClick={() => navigate(tab.path)}
            className={cn('px-4 py-2 text-sm whitespace-nowrap border-b-2 transition-colors',
              location.pathname === tab.path ? 'border-amber-500 text-amber-400' : 'border-transparent text-slate-400 hover:text-slate-200')}>
            {tab.label}
          </button>
        ))}
      </div>
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {sessions.map((s: any) => (
          <div key={s.session_id} className="bg-slate-800/50 rounded-xl p-3">
            <div className="flex items-start gap-3 mb-2">
              {s.thumb && (
                <img src={`https://image.tmdb.org/t/p/w92${s.thumb}`} alt="" className="w-10 h-14 rounded object-cover" />
              )}
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-slate-200 truncate">{s.full_title}</p>
                <p className="text-xs text-slate-500">{s.friendly_name} · {s.player}</p>
                <div className="flex items-center gap-2 mt-1">
                  <Badge variant={s.state === 'playing' ? 'success' : s.state === 'paused' ? 'warning' : 'default'} className="text-[10px]">
                    {s.state}
                  </Badge>
                  <span className="text-xs text-slate-500">{s.stream_video_resolution} {s.stream_video_codec}</span>
        ...
        {!sessions.length && <div className="text-center py-16 text-slate-500">No active streams</div>}
      </div>
    </div>
  )
}
      ...
      {items.map((item: any) => (
        <div key={item.id} className="px-4 py-3">
          ...
          <div className="flex items-center gap-2 mt-0.5 text-xs text-slate-500">
            ...
            {item.started && <span>· {formatDistanceToNow(fromUnixTime(item.started), { addSuffix: true })}</span>}
          </div>
        </div>
      ))}
    </div>
          ...
          <div className="space-y-1.5">
            {(stat.rows ?? []).slice(0, 5).map((row: any, i: number) => (
              <div key={i} className="flex items-center justify-between text-xs">
                <span className="text-slate-400 truncate flex-1">{row.title || row.full_title || row.friendly_name}</span>
                <span className="text-slate-300 ml-2 shrink-0">{row.total_plays ?? row.total_duration} {row.total_plays != null ? 'plays' : 's'}</span>
              </div>
            ))}
          </div>
        </div>
...
export function TautulliApp() {
  const enabled = useServiceEnabled('tautulli')
  if (!enabled) return <NotConfigured serviceName="Tautulli" />
  return (
