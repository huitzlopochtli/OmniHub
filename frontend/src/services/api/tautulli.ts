...
export function createTautulliApi(instanceId: string) {
  return {
  getActivity: () =>
    serviceGet<{ response: { result: string; data: TautulliActivity } }>(instanceId, "/api/v2", { params: { cmd: 'get_activity' } }),

  getHistory: (start = 0, length = 50, mediaType?: string) =>
    serviceGet(instanceId, "/api/v2", { params: { cmd: 'get_history', start, length, ...(mediaType && { media_type: mediaType }) } }),

  getLibraries: () => serviceGet(instanceId, "/api/v2", { params: { cmd: 'get_libraries' } }),

  getLibraryStats: (sectionId: number | string) =>
    serviceGet(instanceId, "/api/v2", { params: { cmd: 'get_library_user_stats', section_id: sectionId } }),

  getPlaysByDate: (timeRange = 30) =>
    serviceGet(instanceId, "/api/v2", { params: { cmd: 'get_plays_by_date', time_range: timeRange } }),

  getPlaysByDayOfWeek: (timeRange = 30) =>
    serviceGet(instanceId, "/api/v2", { params: { cmd: 'get_plays_by_dayofweek', time_range: timeRange } }),

  getPlaysByHourOfDay: (timeRange = 30) =>
    serviceGet(instanceId, "/api/v2", { params: { cmd: 'get_plays_by_hourofday', time_range: timeRange } }),

  getHomeStats: (timeRange = 30) =>
    serviceGet(instanceId, "/api/v2", { params: { cmd: 'get_home_stats', time_range: timeRange, stats_count: 5 } }),
...
export const tautulliApi: ReturnType<typeof createTautulliApi> = new Proxy({} as unknown as ReturnType<typeof createTautulliApi>, {
