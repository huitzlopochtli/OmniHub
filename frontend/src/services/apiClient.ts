...
export function createServiceClient(instanceId: string): AxiosInstance {
  ...
  instance.interceptors.request.use((config) => {
    ...
    if (cfg.username && cfg.password) config.auth = { username: cfg.username, password: cfg.password }
  ...
  instance.interceptors.response.use(
    (res) => res,
    (err) => {
      if (err.code === "ERR_NETWORK") err.message = "Cannot connect to server. Check your URL and network."
      else if (err.response?.status === 401) err.message = "Unauthorized. Check your API key."
      else if (err.response?.status === 403) err.message = "Forbidden. Check your API key permissions."
...
export async function serviceGet<T>(instanceId: string, url: string, config?: AxiosRequestConfig): Promise<T> {
  const res = await createServiceClient(instanceId).get<T>(url, config)
  return res.data
}

export async function servicePost<T>(instanceId: string, url: string, data?: unknown, config?: AxiosRequestConfig): Promise<T> {
  const res = await createServiceClient(instanceId).post<T>(url, data, config)
  return res.data
}

export async function servicePut<T>(instanceId: string, url: string, data?: unknown, config?: AxiosRequestConfig): Promise<T> {
  const res = await createServiceClient(instanceId).put<T>(url, data, config)
  return res.data
}

export async function serviceDelete<T>(instanceId: string, url: string, config?: AxiosRequestConfig): Promise<T> {
