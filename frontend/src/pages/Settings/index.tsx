...
const SERVICE_GROUPS: { label: string; keys: ServiceKey[] }[] = [
  { label: 'Media Managers', keys: ['sonarr', 'radarr', 'lidarr', 'readarr', 'bazarr'] },
  {
    ...
    keys: ['sabnzbd', 'nzbget', 'qbittorrent', 'deluge', 'transmission'],
  },
...
function InstanceCard({
  ...
  meta: (typeof SERVICE_META)[ServiceKey]
}) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 px-3 py-2.5">
        ...
        {instance.baseUrl && (
          <span className="text-xs text-slate-500 truncate max-w-[100px] hidden sm:block">
            {instance.baseUrl}
          </span>
        )}
        ...
        <Switch
          ...
          onChange={(e) =>
            updateInstance(instance.id, { enabled: (e.target as HTMLInputElement).checked })
          }
        />
        <button
    ...
    <div className="space-y-1.5">
      <div className="flex items-center gap-2">
        <button
          ...
          className="flex items-center gap-2 flex-1 text-left py-1 group"
        >
          <span className={cn('shrink-0', meta.color)}>{meta.icon}</span>
      ...
      {collapsed === false && (
        <div className="space-y-2 pl-1">
          {instances.length === 0 ? (
            <button
              onClick={handleAdd}
              className="w-full border-2 border-dashed border-slate-700 rounded-xl py-3 text-sm text-slate-500 hover:border-sky-500/50 hover:text-sky-400 transition-colors flex items-center justify-center gap-2"
            >
              <Plus size={16} />
              Add {meta.label}
            </button>
      ...
      <div>
        ...
        <div className="grid grid-cols-4 gap-2">
          {THEMES.map((t) => (
            <button
              key={t.value}
              onClick={() => setTheme(t.value as 'dark' | 'amoled' | 'light' | 'system')}
              className={cn(
                'rounded-lg overflow-hidden border-2 transition-colors',
                theme === t.value ? 'border-sky-500' : 'border-slate-700',
              )}
            >
        ...
        <div className="flex gap-3">
          {ACCENTS.map((a) => (
            <button
              key={a.value}
              onClick={() =>
                setAccentColor(a.value as 'sky' | 'blue' | 'violet' | 'emerald' | 'orange' | 'rose')
              }
              className={cn(
          ...
          <div className="space-y-6 max-w-2xl mx-auto">
            {SERVICE_GROUPS.map((group) => (
              <div key={group.label}>
                ...
                <div className="space-y-3">
                  {group.keys.map((key) => (
                    <ServiceTypeSection key={key} serviceKey={key} />
                  ))}
                </div>
              </div>
            ))}
