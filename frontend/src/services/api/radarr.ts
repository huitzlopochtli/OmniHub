...
export function createRadarrApi(instanceId: string) {
  return {
    ...
    getMovieById: (id: number) => serviceGet<RadarrMovie>(instanceId, `/api/v3/movie/${id}`, {}),
    ...
    updateMovie: (movie: RadarrMovie) =>
      servicePut<RadarrMovie>(instanceId, `/api/v3/movie/${movie.id}`, movie, {}),
    ...
    searchMovie: (movieIds: number[]) =>
      servicePost(instanceId, '/api/v3/command', { name: 'MoviesSearch', movieIds }, {}),

    refreshMovie: (movieId: number) =>
      servicePost(instanceId, '/api/v3/command', { name: 'RefreshMovie', movieId }, {}),

    // Disk space
    getDiskSpace: () =>
      serviceGet<{ path: string; label: string; freeSpace: number; totalSpace: number }[]>(
        instanceId,
        '/api/v3/diskspace',
        {},
      ),
