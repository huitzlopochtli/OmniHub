...
export function createSabnzbdApi(instanceId: string) {
  return {
    getQueue: (start = 0, limit = 100) =>
      serviceGet<{ queue: SABnzbdQueue }>(instanceId, '/api', apiParams('queue', { start, limit })),
    ...
    pauseJob: (nzoId: string) =>
      serviceGet(instanceId, '/api', apiParams('queue', { name: 'pause', value: nzoId })),

    resumeJob: (nzoId: string) =>
      serviceGet(instanceId, '/api', apiParams('queue', { name: 'resume', value: nzoId })),
    ...
    setSpeedLimit: (value: number | string) =>
      serviceGet(instanceId, '/api', apiParams('config', { name: 'speedlimit', value })),
    ...
    moveJob: (nzoId: string, toPosition: number) =>
      serviceGet(instanceId, '/api', apiParams('switch', { value: nzoId, value2: toPosition })),
    ...
    getCategories: () =>
      serviceGet<{ categories: string[] }>(instanceId, '/api', apiParams('get_cats')),

    getScripts: () =>
      serviceGet<{ scripts: string[] }>(instanceId, '/api', apiParams('get_scripts')),

    getStatus: () => serviceGet(instanceId, '/api', apiParams('fullstatus', { skip_dashboard: 1 })),

    setCategory: (nzoId: string, value: string) =>
      serviceGet(instanceId, '/api', apiParams('change_cat', { value: nzoId, value2: value })),

    setScript: (nzoId: string, script: string) =>
      serviceGet(instanceId, '/api', apiParams('change_script', { value: nzoId, value2: script })),
  }
}
