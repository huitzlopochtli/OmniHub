...
function getConfig() {
  return useSettingsStore.getState().getService(KEY)
}
...
async function gql<T>(query: string, variables?: Record<string, unknown>): Promise<T> {
  const res = await client().post<{ data: T; errors?: { message: string }[] }>(
    '/graphql',
    { query, variables },
  )
...
export const unraidApi = {
  ...
  // Array
  startArray: () =>
    gql(`mutation { startArray }`),

  stopArray: () =>
    gql(`mutation { stopArray }`),

  // Docker containers
  getContainers: () =>
    gql<{ dockerContainers: UnraidDockerContainer[] }>(`
      query {
        dockerContainers {
  ...
  startContainer: (id: string) =>
    gql(`mutation StartContainer($id: String!) { startDockerContainer(id: $id) }`, { id }),

  stopContainer: (id: string) =>
    gql(`mutation StopContainer($id: String!) { stopDockerContainer(id: $id) }`, { id }),

  restartContainer: (id: string) =>
    gql(`mutation RestartContainer($id: String!) { restartDockerContainer(id: $id) }`, { id }),

  pauseContainer: (id: string) =>
    gql(`mutation PauseContainer($id: String!) { pauseDockerContainer(id: $id) }`, { id }),

  unpauseContainer: (id: string) =>
    gql(`mutation UnpauseContainer($id: String!) { unpauseDockerContainer(id: $id) }`, { id }),

  // VMs
  getVMs: () =>
    gql<{ vms: { domain: UnraidVM[] } }>(`
      query {
        vms {
  ...
  startVM: (uuid: string) =>
    gql(`mutation StartVM($uuid: String!) { startVM(uuid: $uuid) }`, { uuid }),

  stopVM: (uuid: string) =>
    gql(`mutation StopVM($uuid: String!) { stopVM(uuid: $uuid) }`, { uuid }),

  pauseVM: (uuid: string) =>
    gql(`mutation PauseVM($uuid: String!) { pauseVM(uuid: $uuid) }`, { uuid }),

  resumeVM: (uuid: string) =>
    gql(`mutation ResumeVM($uuid: String!) { resumeVM(uuid: $uuid) }`, { uuid }),

  // Shares
  getShares: () =>
    gql<{ shares: { share: { name: string; comment: string; freeSize: number; usedSize: number; allocSize: number }[] } }>(`
      query { shares { share { name comment freeSize usedSize allocSize } } }
    `).then((d) => d.shares.share),

  // Notifications
  getNotifications: () =>
    gql<{ notifications: { overview: { unread: { total: number; alert: number; warning: number; normal: number } }; list: unknown[] } }>(`
