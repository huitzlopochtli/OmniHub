  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="shrink-0 p-3 flex gap-2">
        ...
        <button
          onClick={() => setViewMode(viewMode === 'poster' ? 'list' : 'poster')}
...
function PosterCard({
  ...
  return (
    <button
      onClick={onClick}
      className="group flex flex-col rounded-lg overflow-hidden bg-slate-800/50 hover:bg-slate-800 transition-colors text-left"
    >
      <div className="aspect-[2/3] bg-slate-700 relative overflow-hidden">
        ...
        {!movie.hasFile && (
          <div className="absolute inset-0 bg-black/40 flex items-end p-1">
            <span className="text-[10px] bg-yellow-500/90 text-black rounded px-1 font-semibold">
              Missing
            </span>
          </div>
        )}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {movie.ratings?.imdb?.value && (
          <p className="text-xs text-slate-500">IMDb {movie.ratings.imdb.value.toFixed(1)}</p>
        )}
      </div>
